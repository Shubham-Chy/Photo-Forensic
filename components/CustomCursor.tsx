...
const CustomCursor: React.FC = () => {
  ...
  useEffect(() => {
    // Inject the class to hide system cursor only once the React app starts
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      const isClickable =
        ...
        target.closest("a") ||
        target.classList.contains("tech-hover") ||
        target.closest(".tech-hover");
    ...
    const animate = () => {
      ...
      if (ringRef.current) {
        ringRef.current.style.transform = `translate3d(${
          ringPos.current.x
        }px, ${ringPos.current.y}px, 0) translate(-50%, -50%) scale(${
          isClicking ? 0.8 : 1
        })`;
      }
